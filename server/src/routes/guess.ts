...
export async function guessRoutes(fastify: FastifyInstance){
  ...
  fastify.post('/polls/:pollId/games/:gameId/guesses', {
    ...
    if (!participant) {
      return reply.status(402).send({ message: 'You are not allowed to create a guess inside this poll' })
    }
    ...
    if (guess) {
      return reply.status(400).send({ message: 'You already created a guess for this game' })
    }
